...
import { useAddManager } from './useManagers'
import { addManagerSchema } from './manager.types'
...
export function AddManagerModal({ open, onOpenChange }: AddManagerModalProps) {
  ...
  const form = useForm<TAddManagerPayload>({
    resolver: zodResolver(addManagerSchema),
    defaultValues: { full_name: '', email: '', location_id: '' },
  })
  ...
  const handleSubmit = (values: TAddManagerPayload) => {
    setSubmitError(null)
    addManager.mutate(values, {
      onError: (err) => {
        ...
        <div className="flex items-start justify-between border-b border-zinc-200 px-6 py-5">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 bg-kmvmt-bg">
              <Users className="h-4 w-4 text-kmvmt-navy" />
            </div>
            <div>
              <h2 className="text-sm font-semibold text-kmvmt-navy">{MANAGERS_COPY.MODAL_TITLE}</h2>
              <p className="text-xs text-kmvmt-navy/50">{MANAGERS_COPY.MODAL_DESCRIPTION}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <Form {...form}>
          ...
          >
            <div className="space-y-4 px-6 py-5">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs font-medium text-kmvmt-navy">
                      {MANAGERS_COPY.FIELD_NAME}
                    </FormLabel>
                    <FormControl>
                      <Input
                        placeholder={MANAGERS_COPY.FIELD_NAME_PLACEHOLDER}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input
                          type="email"
                          placeholder={MANAGERS_COPY.FIELD_EMAIL_PLACEHOLDER}
            ...
            <div className="flex gap-2.5 border-t border-zinc-200 bg-kmvmt-bg px-6 py-4">
              ...
              >
                {addManager.isPending ? 'Adding…' : MANAGERS_COPY.BTN_SUBMIT}
              </Button>
            </div>
          </form>
        </Form>
